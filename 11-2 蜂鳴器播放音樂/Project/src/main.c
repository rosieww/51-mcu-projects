...
// 天空之城樂譜 
unsigned char code Music[] = { // 儲存在 Flash (只讀不寫)
    // 音符, 時值
    // 1
    P, 4,
    P, 4,
...
void main()
{
    Timer0Init();
    while (1) {
        if (Music[MusicSelect] != 0xFF) { // 如果不是樂譜結束
            FreqSelect = Music[MusicSelect]; // 選擇音符對應的頻率
            MusicSelect++;
            Delay(125 * Music[MusicSelect]); // 選擇音符對應的時值
            MusicSelect++;
...
void Timer0_Routine() interrupt 1
{
    if (FreqTable[FreqSelect]) { // 如果不是休止符
        TL0    = FreqTable[FreqSelect] % 256; // Initial timer value
        TH0    = FreqTable[FreqSelect] / 256; // Initial timer value
        Buzzer = !Buzzer;                     // 蜂鳴器狀態取反
    }
}